...
// @access  Public
router.post('/', [
    check('name', 'Brokerage name is require').not().isEmpty(),
...
router.post('/update', [auth, [
    check('phone', 'Must be a valid phone').isMobilePhone()
]], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    try {
        ...
        brokerage = await Brokerage.findOneAndUpdate(id, { $set: updates }, { new: true, useFindAndModify: false })
        res.json(brokerage);
